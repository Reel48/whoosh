...
export async function setOAuthState(intent: string): Promise<string> {
  const nonce = crypto.randomBytes(16).toString("hex");
  const state = pack({ n: nonce, i: intent });
  const jar = await cookies();
...
export async function consumeOAuthState(state: string): Promise<{ intent: string } | null> {
  const jar = await cookies();
  const stored = jar.get(STATE_COOKIE)?.value;
  jar.delete(STATE_COOKIE);
  if (!stored || stored !== state) return null;
  const payload = unpack<{ n: string; i: string }>(stored);
  return payload ? { intent: payload.i } : null;
}
