-- Stocks are USD-priced; only the Stripe→WB conversion gets the 10× treatment.
-- 1) Undo the position rebase: cost basis is in real-USD cents again.
update invest_position set cost_basis_cents = cost_basis_cents / 10;

-- 2) Remove the 10× multiplier from invest_buy/sell so trades charge the
--    real-market price in WB (1 WB = $1 of WB; stocks priced normally).
...
-- 3) Dividend per-share: previously assumed *10 conversion. Stocks are
--    priced in real USD; dividends in WB = dividend USD (since 1 WB = $1).
--    No change needed in fn_post_dividend (it takes wb_cents_per_share);
--    only the lib wrapper that converted usdPerShare → wb_cents needs to
--    drop the WB_PER_USD multiplier.